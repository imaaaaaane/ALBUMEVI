...
function HamMaddeView() {
  ...
  const fetchHamMaddeler = async () => {
    try {
      const { data, error } = await supabaseClient
        ...
        .order("malzeme_adi", { ascending: true });
  ...
  const baskiList = allAlbums.filter(
    (a) => a?.toLowerCase().includes("baskı") || a?.toLowerCase().includes("baski"),
  );
  ...
  const updateYieldMutation = useMutation({
    mutationFn: async (payload: { urun_adi: string; malzeme_adi: string; cikan_adet: number }) => {
      const { data, error } = await supabaseClient.from("ebatlama_verimi").upsert(
        {
  ...
  const handleUpdate = (
    hm: HamMadde,
    field: "plaka_en" | "plaka_boy" | "plaka_fiyat" | "ilac_fiyati" | "adet",
    val: string,
  ) => {
  ...
  const handleLocalChange = (id: string | number, field: keyof HamMadde, val: string) => {
    ...
    setHamMaddeler((prev) =>
      prev.map((hm) => (hm.id === id ? { ...hm, [field]: numericVal } : hm)),
    );
  };
  ...
  const plakaMaterials = hamMaddeler.filter((m) =>
    PLAKA_NAMES.includes(normalizeString(m.malzeme_adi)),
  );
  const ruloMaterials = hamMaddeler.filter((m) =>
    RULO_NAMES.includes(normalizeString(m.malzeme_adi)),
  );
  const sabitGiderler = hamMaddeler.filter((m) =>
    SABIT_NAMES.includes(normalizeString(m.malzeme_adi)),
  );
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const updatedOrder = [
      ...
      ...hamMaddeler.filter(
        (hm) =>
          !PLAKA_NAMES.includes(normalizeString(hm.malzeme_adi)) &&
          !RULO_NAMES.includes(normalizeString(hm.malzeme_adi)) &&
          !SABIT_NAMES.includes(normalizeString(hm.malzeme_adi)),
      ),
    ];
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[#9E9696] mt-1">
                Malzeme ve gider gruplarının güncel fiyat tanımlamaları.
              </p>
            </div>
          </div>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/5 text-[#9E9696] text-xs font-medium uppercase tracking-wider">
                    ...
                    <th className="px-4 py-4 whitespace-nowrap">Malzeme Adı</th>
                  ...
                  {(provided) => (
                    ...
                    >
                      {plakaMaterials.map((hm, index) => {
                        const adet =
                          Number(
                            isPanoramikSelected ? getYieldValue(hm.malzeme_adi, "panoramik") : 0,
                          ) || 0;
                              ...
                              >
                                <td className="px-2 py-4 text-center">
                                  <div
                                    ...
                                    className="text-[#9E9696] hover:text-white cursor-grab active:cursor-grabbing flex justify-center"
                                  >
                                ...
                                <td className="px-6 py-4 w-32">
                                  <Input
                                    ...
                                    onChange={(e) =>
                                      handleLocalChange(hm.id, "plaka_en", e.target.value)
                                    }
                                ...
                                <td className="px-6 py-4 w-32">
                                  <Input
                                    ...
                                    className="h-10 bg-[#0A0A0A] border-[#1a1a1e] text-center font-medium text-white focus-visible:ring-blue-500"
                                    onChange={(e) =>
                                      handleLocalChange(hm.id, "plaka_boy", e.target.value)
                                    }
                                ...
                                <td className="px-6 py-4 w-40">
                                  <div className="flex items-center gap-2">
                                    <Input
                                      ...
                                      onChange={(e) =>
                                        handleLocalChange(hm.id, "plaka_fiyat", e.target.value)
                                      }
                                      onBlur={(e) =>
                                        handleUpdate(hm, "plaka_fiyat", e.target.value)
                                      }
                                    />
                                  ...
                                  <>
                                    <td className="px-6 py-4 w-32">
                                      <Input
                                        ...
                                        onChange={(e) =>
                                          handleLocalYieldChange(hm.malzeme_adi, e.target.value)
                                        }
                                        onBlur={(e) =>
                                          handleYieldUpdate(
                                            hm.malzeme_adi,
                                    ...
                                    <td className="px-6 py-4 text-white font-bold text-lg">
                                      {birimMaliyet.toLocaleString("tr-TR", {
                  ...
                  {(provided) => (
                    ...
                    >
                      {ruloMaterials.map((hm, index) => {
                        const cikanAdet =
                          Number(isBaskiSelected ? getYieldValue(hm.malzeme_adi, "baski") : 0) || 0;
                                  ...
                                  <>
                                    <td className="px-6 py-4 w-32">
                                      <Input
                                        ...
                                        onBlur={(e) =>
                                          handleYieldUpdate(hm.malzeme_adi, e.target.value, "baski")
                                        }
                                      />
                                    </td>
                                    <td className="px-6 py-4 text-white font-bold text-lg">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/5 text-[#9E9696] text-xs font-medium uppercase tracking-wider">
                    <th className="px-2 py-4 w-8"></th>
                    <th className="px-4 py-4 w-1/3 whitespace-nowrap">İşlem Adı</th>
