...
function OverviewDashboard() {
  ...
  const [upcomingShoots, setUpcomingShoots] = useState<
    { id: string; school: string; days: number }[]
  >([]);
  ...
  useEffect(() => {
    async function fetchAlertData(showLoading = true) {
      try {
        ...
        console.log("Assistant Data:", {
          ...
          orders: ordersData,
          notes: notesData,
          errors: [shootsError, ordersError, notesError],
        });
        ...
        const popArr = Object.keys(packageCounts).map((name) => {
          return {
            ...
            percentage:
              totalPackages > 0 ? Math.round((packageCounts[name] / totalPackages) * 100) : 0,
          };
        });
    ...
    const channel = supabaseClient
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "photo_shoots" }, () =>
  ...
  const highlightedDays = dashboardShoots
    .map((s) => {
  ...
  return (
    <PageTransition className="flex flex-col gap-6 w-full pb-12">
      {/* Title */}
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-white font-sans">Genel Bakış</h1>
          ...
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/60 font-semibold mt-0.5">
                  {currentDate.toLocaleDateString(lang === "TR" ? "tr-TR" : "en-US", {
                    month: "long",
                    year: "numeric",
                  })}
                </p>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-8 h-8 rounded-full border border-white/10 bg-transparent flex items-center justify-center hover:bg-white/10 text-white/70 hover:text-white transition-colors cursor-pointer"
              >
        ...
        >
          <div className="bg-gradient-to-b from-[#1c1a17] to-[#12100e] border border-white/10 border-t-[#D4B8A8]/20 rounded-2xl p-6 flex flex-col flex-1 w-full shadow-2xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {isLoadingAlerts ? (
                <div className="col-span-full text-center py-8 text-sm text-white/40 font-medium">
                  Asistan analiz yapıyor...
                </div>
              ) : (
                <>
                  {upcomingShoots.filter((a) => !dismissedAlerts.includes(a.id)).length === 0 &&
                    ...
                    recentNotes.filter((a) => !dismissedAlerts.includes(`note-${a.id}`)).length ===
                      0 && (
                      <motion.div
                  ...
                  {recentNotes
                    .filter((a) => !dismissedAlerts.includes(`note-${a.id}`))
                    .map((note) => {
      ...
      >
        <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-300 font-semibold mt-0.5">
              Siparişlerin anlık üretim ve lojistik durumları
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative">
          ...
          <div className="bg-[#151515] border border-indigo-500/20 rounded-2xl p-5 relative z-10 flex flex-col items-center text-center shadow-[0_0_15px_rgba(99,102,241,0.05)]">
            ...
            <p className="text-3xl font-bold text-white tracking-tight">
              {funnelCounts?.uretimde || 0}
            </p>
            <span className="text-[10px] text-indigo-400/80 font-bold uppercase tracking-widest mt-1">
              Sipariş
            </span>
          </div>
          ...
          <div className="bg-[#151515] border border-amber-500/20 rounded-2xl p-5 relative z-10 flex flex-col items-center text-center shadow-[0_0_15px_rgba(245,158,11,0.05)]">
            ...
            <p className="text-3xl font-bold text-white tracking-tight">
              {funnelCounts?.kargoyaHazir || 0}
            </p>
            <span className="text-[10px] text-amber-400/80 font-bold uppercase tracking-widest mt-1">
              Sipariş
            </span>
          </div>
          ...
          <div className="bg-[#151515] border border-emerald-500/20 rounded-2xl p-5 relative z-10 flex flex-col items-center text-center shadow-[0_0_15px_rgba(16,185,129,0.05)]">
            ...
            <p className="text-3xl font-bold text-white tracking-tight">
              {funnelCounts?.teslimEdildi || 0}
            </p>
            <span className="text-[10px] text-emerald-400/80 font-bold uppercase tracking-widest mt-1">
              Sipariş
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-white leading-tight">
              Popüler Paketler & Trendler
            </h2>
            <p className="text-xs text-gray-300 font-semibold mt-0.5">
              En çok tercih edilen paketlerin analizi
            </p>
          </div>
        </div>
