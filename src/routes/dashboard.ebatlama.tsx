...
const packPiecesMaxRects = (
  ...
  while (pieces.length > 0) {
    ...
    for (let i = 0; i < pieces.length; i++) {
      ...
      const orientations = [{ w: p.w, h: p.h }];
      ...
      if (bestNodeIndex !== -1) {
        const fr = freeRects[bestNodeIndex];
        const placed = { x: fr.x, y: fr.y, w: placedW, h: placedH, name: p.name };
        placedRects.push(placed);

        const pBicak = { x: placed.x, y: placed.y, w: placed.w + bicak, h: placed.h + bicak };
        const newFreeRects: { x: number; y: number; w: number; h: number }[] = [];

        for (const F of freeRects) {
          if (
            pBicak.x < F.x + F.w &&
            pBicak.x + pBicak.w > F.x &&
            pBicak.y < F.y + F.h &&
            pBicak.y + pBicak.h > F.y
          ) {
            if (pBicak.y > F.y) newFreeRects.push({ x: F.x, y: F.y, w: F.w, h: pBicak.y - F.y });
            if (pBicak.y + pBicak.h < F.y + F.h)
              newFreeRects.push({
                x: F.x,
                y: pBicak.y + pBicak.h,
                w: F.w,
                h: F.y + F.h - (pBicak.y + pBicak.h),
              });
            if (pBicak.x > F.x) newFreeRects.push({ x: F.x, y: F.y, w: pBicak.x - F.x, h: F.h });
            if (pBicak.x + pBicak.w < F.x + F.w)
              newFreeRects.push({
                x: pBicak.x + pBicak.w,
                y: F.y,
                w: F.x + F.w - (pBicak.x + pBicak.w),
                h: F.h,
              });
          } else {
            newFreeRects.push(F);
        ...
        freeRects = newFreeRects.filter((rect, idx) => {
          for (let j = 0; j < newFreeRects.length; j++) {
            if (
              idx !== j &&
              rect.x >= newFreeRects[j].x &&
              rect.y >= newFreeRects[j].y &&
              rect.x + rect.w <= newFreeRects[j].x + newFreeRects[j].w &&
              rect.y + rect.h <= newFreeRects[j].y + newFreeRects[j].h
            )
              return false;
          }
          return true;
...
function EbatlamaView() {
  ...
  const { data: settings } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabaseClient
        .from("ebatlama_settings")
        .select("*")
        .eq("id", 1)
        .single();
      if (error && error.code !== "PGRST116" && error.code !== "42P01") throw error;
      return (
        data || { thickness: "2.7 mm", plate_size: "2800x2100", bicak_payi: "3", auto_rotate: true }
      );
    },
  });
  ...
  const upsertItemMutation = useMutation({
    mutationFn: async (item: CutItem) => {
      ...
      const payload = {
        ...
        en,
        adet,
        plaka_tipi: item.plaka_tipi || plateSize,
      };
    ...
    onSuccess: (data) => {
      if (data) {
        ...
        if (data.oldId) {
          ...
          setItems((prev) =>
            prev.map((item) => (item.id === data.oldId ? { ...item, id: data.id } : item)),
          );
        }
      }
    },
  ...
  // Realtime Sync removed to prevent aggressive state overwriting
  ...
  const handleExportEbatlamaPDF = () => {
    ...
    const data = items.map((item) => {
      ...
      return [item.parcaAdi, item.boy, item.en, item.adet, item.plaka_tipi || "Bilinmiyor"];
    });
    ...
    exportToPDF({
      ...
      columns,
      data,
      summary: [{ label: "Toplam Kesim Adedi", value: totalPieces }],
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      const pw = parseNumber(parts[0]);
      const ph = parseNumber(parts[1]);
    ...
    <div className="p-8 space-y-10 w-full 2xl:max-w-[1600px] mx-auto pb-24 transition-colors duration-300 min-h-screen">
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <Button
                onClick={handleExportEbatlamaPDF}
                className="bg-white/5 border border-white/10 hover:bg-white/10 text-white font-bold h-10 rounded-xl px-4"
              >
                            ...
                            return (
                              ...
                              >
                                <div className="flex flex-col items-center justify-center w-full h-full text-center overflow-hidden p-0.5">
                                  {isTiny ? (
                                    <span
                                      className="font-bold text-white drop-shadow-md leading-none flex flex-col items-center justify-center"
                                      style={{ fontSize: "min(12px, max(6px, 1.5cqw))" }}
                                    >
                                      <span>{rect.w}</span>
                                      <span className="text-[0.7em] opacity-80 my-[1px] leading-none">
                                        x
                                      </span>
                                      <span>{rect.h}</span>
                                    </span>
                                    ...
                                    <span
                                      className="font-bold text-white drop-shadow-md leading-none flex items-center justify-center tracking-tight"
                                      style={{ fontSize: "min(14px, max(7px, 2cqw))" }}
                                    >
                                      {rect.w}x{rect.h}
                                    </span>
                                  )}
      ...
      >
        ...
        >
          {/* Header & Legend */}
          <div
            className={`flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 border-b pb-4 gap-4 ${isDownloading ? "border-gray-200" : "border-gray-200"}`}
          >
            <div className="flex items-center gap-6">
              {activeNames && (
                <div className="bg-[#A67C52]/10 px-4 py-2.5 rounded-xl border border-[#A67C52]/20">
                  <span className="text-2xl font-extrabold text-[#A67C52] uppercase tracking-wider">
                    {activeNames}
                  </span>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
              ...
              >
                ...
                >
                  {optimizationResults.gerekenPlaka}{" "}
                  <span
                    className={`text-xl font-medium ${isDownloading ? "text-gray-400" : "text-slate-400"}`}
                  >
                    adet
                  </span>
                </span>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-2 text-xs font-bold text-white bg-slate-800 hover:bg-[#A67C52] rounded-lg transition-colors shadow-sm"
                          >
                            Sıfırla
                          </button>
                          <button
