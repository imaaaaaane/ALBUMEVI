...
const getFirmPaid = (f: Firm) => f.transactions.filter((tx) => tx.type === "payment").reduce((sum, tx) => sum + tx.amount, 0);
const getFirmDebt = (f: Firm) => f.transactions.filter((tx) => tx.type === "debt").reduce((sum, tx) => sum + tx.amount, 0);
const getFirmRemaining = (f: Firm) => Math.max(0, getFirmDebt(f) - getFirmPaid(f));

const getEmployeePaid = (f: Employee) => f.transactions.filter((tx) => tx.type === "salary_payment" || tx.type === "advance").reduce((sum, tx) => sum + tx.amount, 0);
const getEmployeeDebt = (f: Employee) => f.transactions.filter((tx) => tx.type === "debt_addition").reduce((sum, tx) => sum + tx.amount, 0);
const getEmployeeRemaining = (f: Employee) => Math.max(0, getEmployeeDebt(f) - getEmployeePaid(f));

const getExpensePaid = (f: Expense) => f.transactions.filter((tx) => tx.type === "payment").reduce((sum, tx) => sum + tx.amount, 0);
const getExpenseDebt = (f: Expense) => f.transactions.filter((tx) => tx.type === "debt_addition").reduce((sum, tx) => sum + tx.amount, 0);
const getExpenseRemaining = (f: Expense) => Math.max(0, getExpenseDebt(f) - getExpensePaid(f));

const getSchoolPaid = (f: School) => f.transactions.filter((tx) => tx.type === "payment").reduce((sum, tx) => sum + tx.amount, 0);
const getSchoolDebt = (f: School) => f.transactions.filter((tx) => tx.type === "debt").reduce((sum, tx) => sum + tx.amount, 0);
const getSchoolRemaining = (f: School) => Math.max(0, getSchoolDebt(f) - getSchoolPaid(f));
...
function AccountingDashboard() {
  ...
  const totalRemainingFirms = firmsData.reduce((sum, f) => sum + convertToTry(getFirmRemaining(f), f.currency), 0);
  ...
  // Fetch Common Expenses & Transactions
  const { data: expensesData = [] } = useQuery<Expense[]>({
    ...
    queryFn: async () => {
      ...
      const { data: transactions, error: tErr } = await supabaseClient
        ...
        .select("id, expense_id, transaction_type, amount, description, created_at")
        .order("created_at", { ascending: true });
      if (tErr) return expenses.map(s => ({ id: s.id, name: s.name, currency: s.currency as any, transactions: [] }));

      console.log("Expenses Raw:", expenses);
      console.log("Expense Transactions Raw:", transactions);
      ...
      return expenses.map((s) => ({
        ...
        currency: s.currency as any,
        transactions: transactions
          .filter((t) => String(t.expense_id) === String(s.id))
          .map((t) => ({
            id: t.id,
            date: new Date(t.created_at).toISOString().split("T")[0],
            type: t.transaction_type as ExpenseTransaction["type"],
            amount: Number(t.amount),
            desc: t.description ?? "",
            createdAt: t.created_at,
          })),
  ...
  const totalRemainingExpenses = expensesData.reduce((sum, f) => sum + getExpenseRemaining(f), 0);
  const totalRemainingSchools = schoolsData.reduce((sum, f) => sum + convertToTry(getSchoolRemaining(f), f.currency), 0);

  // Categories list dynamically calculated from database aggregates
  const categories: CategoryItem[] = [
    { id: "cat-1", title: "FİRMALAR", amount: totalRemainingFirms, change: "+8%", isPositive: true },
    { id: "cat-2", title: "ORTAK GİDERLER", amount: totalRemainingExpenses, change: "-3%", isPositive: false },
    { id: "cat-3", title: "MAAŞLAR", amount: totalRemainingEmployees, change: "-2%", isPositive: false },
    { id: "cat-5", title: "OKULLAR", amount: totalRemainingSchools, change: "+15%", isPositive: true },
  ];
  ...
  expensesData.forEach((exp) => {
    exp.transactions.forEach((tx) => {
      combinedTx.push({
        id: tx.id,
        desc: `${exp.name} - ${tx.desc}`,
        date: tx.date,
        amount: -tx.amount, // Always TRY
        category: "cat-2",
        createdAt: tx.createdAt,
      ...
      <AnimatePresence mode="wait">
        {view === "overview" ? (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              ...
              <motion.button
                ...
                className="flex flex-col items-center justify-center min-h-[120px] rounded-2xl border-2 border-dashed border-white/10 hover:border-white/20 bg-white/5 hover:bg-white/10 text-center p-4 cursor-pointer transition-colors"
              >
                <div className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 text-[#9E9696] mb-2">
                  <Plus className="w-5 h-5" />
                </div>
                <span className="text-xs font-semibold text-[#9E9696]">
                  Yeni Bölüm Ekle
                </span>
              </motion.button>
            </div>
          ...
          >
            ...
            {view === "ortak_giderler" && <ExpensesListView expenses={expensesData} onBack={() => setView("overview")} />}
...
function FirmsListView({ firms, exchangeRates, isRatesError, onBack }: FirmsListViewProps) {
  ...
  const getTransactionTryAmount = (tx: FirmTransaction, firmCurrency?: string) => convertToTry(tx.amount, firmCurrency);

  const totalPaid = firms.reduce((sum, f) => {
    const paidTry = f.transactions.filter((tx) => tx.type === "payment").reduce((s, tx) => s + getTransactionTryAmount(tx, f.currency), 0);
    return sum + paidTry;
  }, 0);

  const totalRemaining = firms.reduce((sum, f) => sum + convertToTry(getFirmRemaining(f), f.currency), 0);
  ...
  const handleAddTx = (e: React.FormEvent) => {
    ...
    addTxMutation.mutate({ firm_id: selectedFirmId, type: newTxType, amount: parseFloat(newTxAmount), desc: newTxDesc.trim(), date: newTxDate, currency: firm?.currency });
  };
      ...
      <div className="bg-[#131316] border border-white/5 rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div className="divide-y divide-white/5">
          {filteredFirms.length === 0 ? (
            ...
            filteredFirms.map((f) => {
              const paid = getFirmPaid(f);
              const remaining = getFirmRemaining(f);
              return (
                  ...
                  <div className="flex items-center gap-8 justify-between sm:justify-end flex-1 sm:flex-none">
                    <div className="grid grid-cols-2 gap-6 sm:gap-12 text-right min-w-[200px]">
                      <div>
                        ...
                        <span className="font-mono text-sm font-bold text-[#12B76A] block mt-0.5">{paid.toLocaleString()} {getCurrencySymbol(f.currency)}</span>
                      </div>
                      <div>
                        <span className="text-[10px] font-bold text-[#9E9696] uppercase tracking-wider block">Kalan Borç</span>
                        <span className={`font-mono text-sm font-bold block mt-0.5 ${remaining > 0 ? "text-[#A67C52]" : "text-[#9E9696]"}`}>{remaining.toLocaleString()} {getCurrencySymbol(f.currency)}</span>
                      </div>
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); if (confirm("Bu firmayı silmek istediğinizden emin misiniz?")) deleteFirmMutation.mutate(f.id); }} className="p-2 rounded-lg hover:bg-white/5 text-[#9E9696] hover:text-[#A67C52] transition-colors cursor-pointer ml-4">
      ...
      <Dialog open={!!breakdownType} onOpenChange={(open) => !open && setBreakdownType(null)}>
        <DialogContent className="border-border bg-[#131316] text-white rounded-2xl max-w-md">
          ...
          <div className="space-y-3 pt-4">
            {firms.map((f) => {
              const amount = breakdownType === "paid" ? getFirmPaid(f) : getFirmRemaining(f);
              if(amount === 0) return null;
              const hasForeignCurrency = f.currency && f.currency !== "TRY";
              const convertedAmount = convertToTry(amount, f.currency);
              return (
                <div key={f.id} className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-xl">
                  ...
                  <div className="text-right">
                    <span className={`font-mono font-bold text-sm block ${breakdownType === "paid" ? "text-[#12B76A]" : "text-[#A67C52]"}`}>{amount.toLocaleString()} {getCurrencySymbol(f.currency)}</span>
                    {hasForeignCurrency && <span className="font-mono text-xs text-[#9E9696] block mt-0.5">({Math.round(convertedAmount).toLocaleString()} ₺)</span>}
                  </div>
                </div>
              );
            })}
            ...
            <div className="flex flex-col h-full max-h-[85vh]">
              ...
              <div className="grid grid-cols-3 gap-3 px-6 py-4 bg-white/[0.01] border-b border-white/5">
                <div className="p-3 bg-white/5 border border-white/5 rounded-xl text-center">
                  ...
                  <span className="font-mono text-sm font-bold text-[#12B76A] block mt-0.5">{getFirmPaid(selectedFirm).toLocaleString()} {getCurrencySymbol(selectedFirm.currency)}</span>
                </div>
                <div className="p-3 bg-white/5 border border-white/5 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-[#9E9696] uppercase tracking-wider block">Toplam Borç</span>
                  <span className="font-mono text-sm font-bold text-white block mt-0.5">{getFirmDebt(selectedFirm).toLocaleString()} {getCurrencySymbol(selectedFirm.currency)}</span>
                </div>
                <div className="p-3 bg-[#A67C52]/10 border border-[#A67C52]/20 rounded-xl text-center">
                  ...
                  <span className="font-mono text-sm font-bold text-[#A67C52] block mt-0.5">{getFirmRemaining(selectedFirm).toLocaleString()} {getCurrencySymbol(selectedFirm.currency)}</span>
                </div>
              </div>
                  ...
                  {selectedFirm.transactions.map((tx) => {
                    ...
                    return (
                      <div key={tx.id} className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-xl">
                        ...
                        <span className={`font-mono text-sm font-bold ${isTaken ? "text-[#A67C52]" : "text-[#12B76A]"}`}>{isTaken ? "+" : "-"}{tx.amount.toLocaleString()} {getCurrencySymbol(selectedFirm.currency)}</span>
                      </div>
                    );
                  })}
                </div>
    ...
    mutationFn: async (input: { name: string; taken: number; rest: number }) => {
      ...
      if (input.taken > 0) {
        await supabaseClient.from("salary_transactions").insert({
          employee_id: supplier.id, transaction_type: "debt_addition", amount: input.taken, description: "İlk Maaş Tahakkuku"
        });
      }
      if (input.rest > 0) {
        await supabaseClient.from("salary_transactions").insert({
          employee_id: supplier.id, transaction_type: "salary_payment", amount: input.rest, description: "İlk Maaş Ödemesi"
        });
      }
      return supplier;
    },
              ...
              return (
                <motion.div key={f.id} whileHover={{ scale: 1.005, x: 2, backgroundColor: "rgba(255,255,255,0.02)" }} whileTap={{ scale: 0.995 }} onClick={() => setSelectedEmployeeId(f.id)} className="flex flex-col sm:flex-row sm:items-center justify-between p-6 cursor-pointer gap-4 transition-colors">
                  ...
                  <div className="flex items-center gap-8 justify-between sm:justify-end flex-1 sm:flex-none">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); if (confirm("Bu personeli silmek istediğinizden emin misiniz?")) deleteEmployeeMutation.mutate(f.id); }} className="p-2 rounded-lg hover:bg-white/5 text-[#9E9696] hover:text-[#A67C52] transition-colors cursor-pointer ml-4">
                      <Trash2 className="w-4.5 h-4.5" />
                    </button>
                  </div>
                </motion.div>
...
function ExpensesListView({ expenses, onBack }: ExpensesListViewProps) {
  const qc = useQueryClient();
  const [searchQuery, setSearchQuery] = useState("");
  ...
  const [newTxType, setNewTxType] = useState<"debt_addition" | "payment">("debt_addition");
  const [newTxAmount, setNewTxAmount] = useState("");
  const [newTxDate, setNewTxDate] = useState(new Date().toISOString().split("T")[0]);
  const [newTxDesc, setNewTxDesc] = useState(""); 
  ...
  const totalPaid = expenses.reduce((sum, f) => sum + getExpensePaid(f), 0);
  const totalRemaining = expenses.reduce((sum, f) => sum + getExpenseRemaining(f), 0);
  ...
  const addExpenseMutation = useMutation({
    mutationFn: async (input: { name: string; taken: number; rest: number }) => {
      ...
      if (input.taken > 0) {
        await supabaseClient.from("expense_transactions").insert({
          expense_id: supplier.id, transaction_type: "debt_addition", amount: input.taken, description: "İlk Borç Kaydı"
        });
      }
      if (input.rest > 0) {
        await supabaseClient.from("expense_transactions").insert({
          expense_id: supplier.id, transaction_type: "payment", amount: input.rest, description: "İlk Ödeme Kaydı"
        });
      }
      return supplier;
    },
  ...
  const deleteExpenseMutation = useMutation({
    mutationFn: async (id: string) => {
      await supabaseClient.from("expense_transactions").delete().eq("expense_id", id);
  ...
  const addTxMutation = useMutation({
    mutationFn: async (input: { expense_id: string; type: string; amount: number; desc: string; date: string }) => {
      await supabaseClient.from("expense_transactions").insert({
        expense_id: input.expense_id, transaction_type: input.type, amount: input.amount, description: input.desc, created_at: new Date(input.date).toISOString()
      });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["expenses_ledger"] });
  ...
  const handleAddTx = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedExpenseId || !newTxAmount || !newTxDesc.trim()) return;
    addTxMutation.mutate({ expense_id: selectedExpenseId, type: newTxType, amount: parseFloat(newTxAmount), desc: newTxDesc.trim(), date: newTxDate });
  };
    ...
    <div className="space-y-8">
        ...
        <div className="divide-y divide-white/5">
          {filteredExpenses.length === 0 ? (
            ...
            filteredExpenses.map((f) => {
              const paid = getExpensePaid(f);
              const remaining = getExpenseRemaining(f);
              return (
                <motion.div key={f.id} whileHover={{ scale: 1.005, x: 2, backgroundColor: "rgba(255,255,255,0.02)" }} whileTap={{ scale: 0.995 }} onClick={() => setSelectedExpenseId(f.id)} className="flex flex-col sm:flex-row sm:items-center justify-between p-6 cursor-pointer gap-4 transition-colors">
                  ...
                  <div className="flex items-center gap-8 justify-between sm:justify-end flex-1 sm:flex-none">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); if (confirm("Bu gider merkezini silmek istediğinizden emin misiniz?")) deleteExpenseMutation.mutate(f.id); }} className="p-2 rounded-lg hover:bg-white/5 text-[#9E9696] hover:text-[#A67C52] transition-colors cursor-pointer ml-4">
                      <Trash2 className="w-4.5 h-4.5" />
                    </button>
                  </div>
                </motion.div>
      ...
      <Dialog open={!!breakdownType} onOpenChange={(open) => !open && setBreakdownType(null)}>
        <DialogContent className="border-border bg-[#131316] text-white rounded-2xl max-w-md">
          ...
          <div className="space-y-3 pt-4">
            {expenses.map((f) => {
              const amount = breakdownType === "paid" ? getExpensePaid(f) : getExpenseRemaining(f);
              if(amount === 0) return null;
              return (
                <div key={f.id} className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-xl">
                  <span className="font-bold text-white text-sm">{f.name}</span>
        ...
        <DialogContent className="border-border bg-[#131316] text-white rounded-3xl max-w-lg p-0 overflow-hidden">
          {selectedExpense && (
            <div className="flex flex-col h-full max-h-[85vh]">
              <div className="p-6 bg-white/[0.02] border-b border-white/5 flex justify-between items-start">
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-[#A67C52] uppercase tracking-widest">GİDER DETAY PANELİ</span>
                  <h3 className="text-xl font-extrabold text-white leading-tight">{selectedExpense.name}</h3>
                </div>
                <button onClick={() => setSelectedExpenseId(null)} className="p-1.5 rounded-lg hover:bg-white/5 text-[#9E9696] hover:text-white transition-colors cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
              <div className="grid grid-cols-3 gap-3 px-6 py-4 bg-white/[0.01] border-b border-white/5">
                <div className="p-3 bg-white/5 border border-white/5 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-[#9E9696] uppercase tracking-wider block">Ödenen</span>
                  <span className="font-mono text-sm font-bold text-[#12B76A] block mt-0.5">{getExpensePaid(selectedExpense).toLocaleString()} ₺</span>
                </div>
                <div className="p-3 bg-white/5 border border-white/5 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-[#9E9696] uppercase tracking-wider block">Toplam Gider</span>
                  <span className="font-mono text-sm font-bold text-white block mt-0.5">{getExpenseDebt(selectedExpense).toLocaleString()} ₺</span>
                </div>
                <div className="p-3 bg-[#A67C52]/10 border border-[#A67C52]/20 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-[#A67C52] uppercase tracking-wider block">Kalan Borç</span>
                  <span className="font-mono text-sm font-bold text-[#A67C52] block mt-0.5">{getExpenseRemaining(selectedExpense).toLocaleString()} ₺</span>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  {selectedExpense.transactions.map((tx) => {
                    ...
                    return (
                      <div key={tx.id} className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-xl">
                        ...
                        <span className={`font-mono text-sm font-bold ${isTaken ? "text-[#A67C52]" : "text-[#12B76A]"}`}>{isTaken ? "+" : "-"}{tx.amount.toLocaleString()} ₺</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <DialogContent className="border-border bg-[#131316] text-white rounded-2xl max-w-sm">
          ...
          <form onSubmit={handleAddTx} className="space-y-4 pt-2">
            <div className="space-y-2">
              <Label className="text-sm font-semibold text-gray-300">İşlem Tipi</Label>
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setNewTxType("debt_addition")} className={`h-11 rounded-xl font-bold text-xs border ${newTxType === "debt_addition" ? "bg-[#A67C52]/10 border-[#A67C52] text-[#A67C52]" : "bg-white/5 border-white/5 text-[#9E9696]"}`}>Gider Ekle (Borç)</button>
                <button type="button" onClick={() => setNewTxType("payment")} className={`h-11 rounded-xl font-bold text-xs border ${newTxType === "payment" ? "bg-[#12B76A]/10 border-[#12B76A] text-[#12B76A]" : "bg-white/5 border-white/5 text-[#9E9696]"}`}>Ödeme Yapıldı</button>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
...
function OkullarListView({ schools, exchangeRates, isRatesError, onBack }: OkullarListViewProps) {
  ...
  const totalRemaining = schools.reduce((sum, f) => sum + convertToTry(getSchoolRemaining(f), f.currency), 0);
    ...
    mutationFn: async (input: { name: string; currency: string; taken: number; rest: number }) => {
      ...
      if (input.taken > 0) {
        await supabaseClient.from("school_transactions").insert({
          school_id: supplier.id, transaction_type: "debt", amount: input.taken, description: "İlk Borç Kaydı", currency: input.currency
        });
      }
      if (input.rest > 0) {
        await supabaseClient.from("school_transactions").insert({
          school_id: supplier.id, transaction_type: "payment", amount: input.rest, description: "İlk Ödeme Kaydı", currency: input.currency
        });
      }
      return supplier;
    },
    ...
    <div className="space-y-8">
        ...
        <div className="divide-y divide-white/5">
          {filteredSchools.length === 0 ? (
            ...
            filteredSchools.map((f) => {
              const paid = getSchoolPaid(f);
              const remaining = getSchoolRemaining(f);
              return (
                <motion.div key={f.id} whileHover={{ scale: 1.005, x: 2, backgroundColor: "rgba(255,255,255,0.02)" }} whileTap={{ scale: 0.995 }} onClick={() => setSelectedSchoolId(f.id)} className="flex flex-col sm:flex-row sm:items-center justify-between p-6 cursor-pointer gap-4 transition-colors">
                  ...
                  <div className="flex items-center gap-8 justify-between sm:justify-end flex-1 sm:flex-none">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); if (confirm("Bu okulu silmek istediğinizden emin misiniz?")) deleteSchoolMutation.mutate(f.id); }} className="p-2 rounded-lg hover:bg-white/5 text-[#9E9696] hover:text-[#A67C52] transition-colors cursor-pointer ml-4">
                      <Trash2 className="w-4.5 h-4.5" />
                    </button>
                  </div>
                </motion.div>
      ...
      <Dialog open={!!breakdownType} onOpenChange={(open) => !open && setBreakdownType(null)}>
        <DialogContent className="border-border bg-[#131316] text-white rounded-2xl max-w-md">
          ...
          <div className="space-y-3 pt-4">
            {schools.map((f) => {
              const amount = breakdownType === "paid" ? getSchoolPaid(f) : getSchoolRemaining(f);
              if(amount === 0) return null;
              const hasForeignCurrency = f.currency && f.currency !== "TRY";
              const convertedAmount = convertToTry(amount, f.currency);
              return (
          ...
          {selectedSchool && (
            <div className="flex flex-col h-full max-h-[85vh]">
              ...
              <div className="grid grid-cols-3 gap-3 px-6 py-4 bg-white/[0.01] border-b border-white/5">
                <div className="p-3 bg-white/5 border border-white/5 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-[#9E9696] uppercase tracking-wider block">Ödenen</span>
                  <span className="font-mono text-sm font-bold text-[#12B76A] block mt-0.5">{getSchoolPaid(selectedSchool).toLocaleString()} {getCurrencySymbol(selectedSchool.currency)}</span>
                </div>
                <div className="p-3 bg-white/5 border border-white/5 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-[#9E9696] uppercase tracking-wider block">Toplam Borç</span>
                  <span className="font-mono text-sm font-bold text-white block mt-0.5">{getSchoolDebt(selectedSchool).toLocaleString()} {getCurrencySymbol(selectedSchool.currency)}</span>
                </div>
                <div className="p-3 bg-[#A67C52]/10 border border-[#A67C52]/20 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-[#A67C52] uppercase tracking-wider block">Kalan Borç</span>
                  <span className="font-mono text-sm font-bold text-[#A67C52] block mt-0.5">{getSchoolRemaining(selectedSchool).toLocaleString()} {getCurrencySymbol(selectedSchool.currency)}</span>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  {selectedSchool.transactions.map((tx) => {
                    ...
                    return (
                      <div key={tx.id} className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-xl">
                        ...
                        <span className={`font-mono text-sm font-bold ${isTaken ? "text-[#A67C52]" : "text-[#12B76A]"}`}>{isTaken ? "+" : "-"}{tx.amount.toLocaleString()} {getCurrencySymbol(selectedSchool.currency)}</span>
                      </div>
                    );
                  })}
                </div>
