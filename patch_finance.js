...
const targetContent = `      const { data: transactions, error: tErr } = await supabaseClient
...
const replacementContent = `      const { data: transactions, error: tErr } = await supabaseClient
        .from("school_transactions")
        .select("id, school_id, transaction_type, amount, description, created_at, currency")
        .order("created_at", { ascending: true });
      if (tErr) return schools.map(s => ({ id: s.id, name: s.name, currency: s.currency as any, transactions: [] }));
...
if (content.includes(targetContent)) {
  ...
  console.log("Successfully patched schools_ledger query.");
} else {
